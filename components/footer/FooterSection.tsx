'use client';


import { Reveal } from "../reveal";
import FooterCard from "./FooterCard";
import * as FooterCardContent from "./FooterContent";
import { BrandTan } from "@/components/icons";
import { Card } from "@nextui-org/react";


const Footer = () => {
    return (
        <section className="flex items-center bg-green-100 h-[250px]">
            <div className="grid max-w-[1250px] mx-auto">
                <div className="grid grid-cols-11 h-full items-center">
                    <Reveal hiddenVariant="hiddenY" visibleVariant="visibleY" styling="col-span-3 justify-self-start overflow-hidden" delay={0.2 + Math.random() * 0.4}>
                        <FooterCard
                            header={<BrandTan />}
                            content1={<FooterCardContent.BrandContent1 />}
                        />
                    </Reveal>
                    <Reveal hiddenVariant="hiddenFade" visibleVariant="visibleFade" delay={0.2 + Math.random() * 0.4}>
                        <Card.Divider className="rotate-90" />
                    </Reveal>
                    <Reveal hiddenVariant="hiddenY" visibleVariant="visibleY" styling="col-start-5 col-span-3 overflow-hidden" delay={0.2 + Math.random() * 0.4}>
                        <FooterCard
                            header={<FooterCardContent.ContactUsHeader />}
                            content1={<FooterCardContent.ContactUsContent1 />}
                            content2={<FooterCardContent.ContactUsContent2 />}
                        />
                    </Reveal>
                    <Reveal hiddenVariant="hiddenFade" visibleVariant="visibleFade" delay={0.2 + Math.random() * 0.4}>
                        <Card.Divider className="rotate-90" />
                    </Reveal>
                    <Reveal hiddenVariant="hiddenY" visibleVariant="visibleY" styling="col-start-9 col-span-3 overflow-hidden" delay={0.2 + Math.random() * 0.4}>
                        <FooterCard
                            header={<FooterCardContent.HoursHeader />}
                            content1={<FooterCardContent.HoursContent1 />}
                            content2={<FooterCardContent.HoursContent2 />}
                            styling={"col-start-9 col-span-3"}
                        />
                    </Reveal>
                </div>
            </div>
        </section>
    )
};

export default Footer;